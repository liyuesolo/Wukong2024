...
template<int dim = 2>
void addForceEntry(Eigen::VectorXd& residual, 
...
template<int dim = 2>
void getSubVector(const Eigen::VectorXd& _vector, 
    const std::vector<int>& vtx_idx, 
...
void addHessianBlock(
    std::vector<Eigen::Triplet<T>>& triplets,
    const std::vector<int>& vtx_idx, 
    const Eigen::MatrixXd& hessian_block,
    int shift_row = 0, int shift_col=0)
{
