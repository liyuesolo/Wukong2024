...
void Rod::setupBishopFrame()
{
    ...
    reference_angles =
        full_states.segment(theta_dof_start_offset, indices.size() - 1);
    TV prev_tangent;
    iterateSegments(
        [&](int node_i, int node_j, int segment)
        {
            TV xi, xj;
            x(node_i, xi);
            x(node_j, xj);
            ...
            else
            {
                u = parallelTransportOrthonormalVector(
                    reference_frame_us[segment - 1], prev_tangent, tangent);
            }
            prev_tangent = tangent;
            prev_tangents[segment] = tangent;
            reference_frame_us[segment] = u;
            if (segment)
                reference_twist[segment] =
                    computeReferenceTwist(tangent, prev_tangent, segment);
        });
    ...
    iterateSegments(
        [&](int node_i, int node_j, int segment)
        {
            ...
            reference_frame_us[segment] = parallelTransportOrthonormalVector(
                reference_frame_us[segment], tangent_last_time_step, tangent);
            if (segment)
                reference_twist[segment] =
                    computeReferenceTwist(tangent, prev_tangent, segment);
            // std::cout << computeReferenceTwist(tangent, prev_tangent,
            // segment) << std::endl;
            prev_tangent = tangent;
            prev_tangents[segment] = tangent;
            // reference_twist[segment] = computeReferenceTwist(tangent,
            // prev_tangent, segment);
        });
}
...
T Rod::computeReferenceTwist(const TV& tangent, const TV& prev_tangent,
                             int rod_idx)
{
    TV ut = parallelTransportOrthonormalVector(reference_frame_us[rod_idx - 1],
                                               prev_tangent, tangent);
...
void Rod::markDoF(std::vector<Entry>& w_entry, int& dof_cnt)
{
    ...
    if (dof_node_location.size())
    {
        if (dof_node_location.front() != 0)
        {
            ...
            // std::cout << "node " <<  indices.front() << " added all dof " <<
            // std::endl;
            for (int d = 0; d < 3 + 1; d++)
            {
    ...
    for (int i = 0; i < dof_node_location.size(); i++)
    {
        ...
        // std::cout << "left node " << indices[loop_id] << " right node " <<
        // indices[dof_node_location[i]] << std::endl;
        ...
        for (int j = loop_id + 1; j < dof_node_location[i]; j++)
        {
            ...
            {
                reduced_map[offset[d]] = dof_cnt;
                // std::cout << "add lagrangian entry to " << offset[d] << " "
                // << dof_cnt << std::endl;
                w_entry.push_back(Eigen::Triplet<T>(offset[d], dof_cnt++, 1.0));
            }
            ...
            // std::cout << "add eulerian entry to " << offset[3] << " " <<
            // reduced_map[offset_left_node[3]] << std::endl;
            w_entry.push_back(Eigen::Triplet<T>(
                offset[3], reduced_map[offset_left_node[3]], 1.0 - alpha));
            w_entry.push_back(Eigen::Triplet<T>(
                offset[3], reduced_map[offset_right_node[3]], alpha));
        }
    ...
    if (loop_id != indices.size() - 1 && !closed)
    {
        ...
        // std::cout << "node " <<  indices.back() << " added all dof " <<
        // std::endl;
        for (int d = 0; d < 3 + 1; d++)
        {
            reduced_map[offset[d]] = dof_cnt;
...
bool Rod::isFixedNodeForPrinting(int node_idx, int rod_idx)
{
    ...
    if (iter == dof_node_idx.end())
    {
        ...
        // for (int location : dof_node_location)
        //     std::cout << fixed_by_crossing[location] << " " << location <<
        //     std::endl;
        // std::cout << "dd" << std::endl;
        // std::cout << left_node << " " << right_node << " " << rod_idx <<
        // std::endl; std::cout << fixed_by_crossing[left_node] << " " <<
        // fixed_by_crossing[right_node] << std::endl; std::getchar();

        // if((fixed_by_crossing[left_node] || left_node == 0) &&
        // (fixed_by_crossing[right_node] || right_node == indices.size() - 1))
        //     return true;
        // else
        //     return false;
